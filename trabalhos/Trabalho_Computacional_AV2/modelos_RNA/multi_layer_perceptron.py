...
class MLP:
    def __init__(self, input_size, q, m, η=0.0001, ϵ=0.0001, tolleration=2, epochs=50):
        ...
        for weight in self.__weights:
            copy.append(weight.copy())

        self.__weights_history.append(copy)
    ...
    # PROFESSOR ERA AQUI QUE TAVA ERRADO E TAVA CAUSANDO OVERFLOW!!!
    def __sigmoid_logistic_derivative(self, x):
        s = self.__sigmoid_logistic(x)
        return s * (1 - s)
    ...
    def __backward(self, x_sample, d):
        """
        ...
        while True:
            for epoch in range(self.__epochs):
                for i in range(X.shape[0]):  # Passa cada amostra individualmente
                    ...
                    # Atualiza os pesos
                    self.__backward(x_sample, y_sample)
                # Calcula o EQM
                EQM = self.__EQM(X, y)
                ...
                if epoch > 1:
                    ...
                    if epoch % 5 == 0:
                        ...
                        EQM_validation = self.__EQM(X_validation, y_validation)
